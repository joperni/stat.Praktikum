...
ppseven_day_alter = dt_seven_day_inz_melt %>%
  ...
  scale_color_manual(values = farben, name = "Altersgruppe", labels = c(names(farben)[1:6])) +
  ...
  scale_x_date(breaks = as.Date(c("2020-01-01", "2020-04-01", "2020-07-01", "2020-10-01", "2021-01-01")),
               date_labels = "%d.%m.%y") +
  geom_vline(aes(xintercept=as.numeric(feiertag)), color = "blue", size=0.5) +
  theme_bw() +
